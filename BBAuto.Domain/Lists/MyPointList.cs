...
{
  public class MyPointList : MainList<MyPoint>
  {
    private static MyPointList _uniqueInstance;

    public static MyPointList getInstance()
    {
      return _uniqueInstance ?? (_uniqueInstance = new MyPointList());
    }
    ...
    {
      if (_list.Exists(item => item == myPoint))
        return;
    ...
    {
      var listNew = _list.Where(item => item.RegionID == idRegion).ToList();
    ...
    public DataTable ToDataTableWithoutExists(int idRegion, MyPoint myPoint1)
    {
      ...
      var listNew = _list.Where(item => item.RegionID == idRegion && !routeList.Exists(myPoint1, item) &&
                                       item != myPoint1).ToList();
