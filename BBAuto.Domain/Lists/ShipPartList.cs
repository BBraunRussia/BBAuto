...
{
  public class ShipPartList : MainList<ShipPart>
  {
    private static ShipPartList _uniqueInstance;
    
    public static ShipPartList getInstance()
    {
      return _uniqueInstance ?? (_uniqueInstance = new ShipPartList());
    }
    ...
    {
      var shipParts = _list.Where(item => item.Car.ID == car.ID);
